...
const HomePage = () => {
  ...
  const defaultTab: CategoryTab = location.pathname === "/manhwa" ? "MANHWA" : "ALL";
  ...
  const availableGenres = useMemo(
    () => ["ALL", ...Array.from(new Set(titles.map((t) => t.genre || "General"))).sort()],
    [titles]
  );
  ...
  const filtered = useMemo(() => {
    const result = titles.filter((t) => {
      ...
      const matchesGenre = genreFilter === "ALL" || (t.genre || "General") === genreFilter;
    ...
    <div className="min-h-screen bg-[#0a0a14]">
      ...
      <section className="relative overflow-hidden bg-mesh">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16">
          <div className="text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs font-medium mb-6 animate-fade-in">
              <span className="w-1.5 h-1.5 rounded-full bg-rose-400 animate-pulse" />
              Community-driven anime, manhwa & movies tracking
            </div>
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black text-white leading-tight tracking-tight animate-fade-in">
              Track. Rate.{" "}
              <span className="bg-gradient-to-r from-rose-400 via-orange-400 to-amber-400 bg-clip-text text-transparent">
                Discover.
              </span>
            </h1>
            <p className="mt-6 text-lg sm:text-xl text-gray-400 leading-relaxed animate-fade-in animate-fade-in-delay-1">
              Your <span className="text-white font-medium">anime, manhwa & movies universe</span>, organized.
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-10 animate-fade-in animate-fade-in-delay-2">
              ...
              {!isAuthenticated && (
                <Link
                  to="/register"
                  className="w-full sm:w-auto px-8 py-3.5 border border-white/10 hover:border-white/20 text-gray-300 hover:text-white font-medium rounded-xl transition-all text-center hover:bg-white/5"
                >
                  Create Account
                </Link>
              )}
            ...
            <div className="grid grid-cols-4 gap-8 mt-16 animate-fade-in animate-fade-in-delay-3">
              <div>
                <div className="text-2xl sm:text-3xl font-bold text-white">{animeCount}</div>
                <div className="text-sm text-gray-500 mt-1">Anime</div>
              </div>
              <div>
                <div className="text-2xl sm:text-3xl font-bold text-white">{manhwaCount}</div>
                <div className="text-sm text-gray-500 mt-1">Manhwa</div>
              </div>
              <div>
                <div className="text-2xl sm:text-3xl font-bold text-white">{movieCount}</div>
        ...
        <div className="h-24 bg-gradient-to-b from-transparent to-[#0a0a14]" />
      </section>
      ...
      <section id="browse" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        {isAuthenticated && recommendedLoading && (
          <div className="mb-8">
            <h2 className="text-xl font-bold text-white mb-3">Recommended for you</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="rounded-xl border border-white/5 bg-surface-2 p-4">
        ...
        {isAuthenticated && !recommendedLoading && recommended.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xl font-bold text-white mb-3">Recommended for you</h2>
            <p className="text-sm text-gray-500 mb-4">Based on your highly rated genres and watch preferences.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {recommended.slice(0, 4).map((title) => (
                ...
                >
                  <Link to={`/anime/${title.id}`} className="rounded-xl border border-white/5 bg-surface-2 p-4 hover:border-rose-500/30 transition block">
                    <div className="text-white font-semibold line-clamp-1">{title.name}</div>
                    <div className="text-xs text-gray-500 mt-1">{title.genre || "General"} · {title.releaseYear}</div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-surface-2 border border-white/5 rounded-xl pl-12 pr-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-rose-500/50 focus:ring-1 focus:ring-rose-500/25 transition"
            />
          </div>
          <select
            value={sort}
            onChange={(e) => setSort(e.target.value as SortOption)}
            className="bg-surface-2 border border-white/5 rounded-xl px-4 py-3 text-gray-300 focus:outline-none focus:border-rose-500/50 cursor-pointer min-w-[160px]"
          >
          ...
          <select
            ...
            className="bg-surface-2 border border-white/5 rounded-xl px-4 py-3 text-gray-300 focus:outline-none focus:border-rose-500/50 cursor-pointer min-w-[150px]"
          >
          ...
          <select
            ...
            className="bg-surface-2 border border-white/5 rounded-xl px-4 py-3 text-gray-300 focus:outline-none focus:border-rose-500/50 cursor-pointer min-w-[140px]"
          >
          ...
          <select
            ...
            className="bg-surface-2 border border-white/5 rounded-xl px-4 py-3 text-gray-300 focus:outline-none focus:border-rose-500/50 cursor-pointer min-w-[130px]"
          >
            {availableYears.map((year) => (
              <option key={year} value={year}>{year === "ALL" ? "All Years" : year}</option>
            ))}
          </select>
          <span className="text-sm text-gray-600 whitespace-nowrap self-center">
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-20">
            ...
            {!search && !isAuthenticated && (
              <Link
                to="/login"
                className="inline-flex px-6 py-2.5 border border-white/10 hover:border-white/20 text-gray-300 hover:text-white rounded-xl transition-all"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((title, idx) => (
              ...
              >
                <Link
                  to={`/anime/${title.id}`}
                  className="glow-card group relative rounded-2xl overflow-hidden bg-surface-2 border border-white/5 hover:border-rose-500/30 block"
                >
                  ...
                  <div className="p-5 pt-2">
                    <h3 className="text-lg font-bold text-white group-hover:text-rose-300 transition-colors leading-snug mb-2">
                      {title.name}
                    </h3>
