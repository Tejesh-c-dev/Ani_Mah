...
const AddAnimePage = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await createTitle({ name, description, releaseYear: year, type, genre, image });
      if (res.success) {
        navigate("/");
      } else {
    ...
    <div className="min-h-screen bg-[#0a0a14] px-4 py-12">
      <div className="max-w-2xl mx-auto animate-fade-in">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-600 mb-8">
          ...
          <span>/</span>
          <span className="text-gray-400">Add Title</span>
        </div>

        <div className="bg-surface-2 rounded-2xl border border-white/5 overflow-hidden shadow-2xl shadow-black/40">
          {/* Header with gradient */}
          <div className="bg-gradient-to-r from-rose-600/10 via-orange-500/5 to-transparent px-8 py-6 border-b border-white/5">
            ...
            <p className="text-gray-500 text-sm mt-1">Contribute to the community by adding anime, manhwa, or movies</p>
          </div>
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Type selector */}
              <div>
                <label className="block text-gray-400 text-xs font-medium uppercase tracking-wider mb-3">Type</label>
              ...
              <div>
                <label className="block text-gray-400 text-xs font-medium uppercase tracking-wider mb-2">Title Name</label>
                <input
                  type="text"
                  value={name}
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                />
                <p className="text-gray-700 text-xs mt-1.5">{description.length}/500 characters</p>
              </div>

              <div>
                <label className="block text-gray-400 text-xs font-medium uppercase tracking-wider mb-2">Genre</label>
                <select
                  value={genre}
                  onChange={(e) => setGenre(e.target.value)}
                  className="w-full bg-[#0a0a14] border border-white/5 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-rose-500/50 focus:ring-1 focus:ring-rose-500/25 transition"
                >
                  {[
                    "Action",
                    "Adventure",
              ...
              <div>
                <label className="block text-gray-400 text-xs font-medium uppercase tracking-wider mb-2">Release Year</label>
                <input
                  type="number"
                  value={releaseYear}
              ...
              {/* Image upload */}
              <div>
                <label className="block text-gray-400 text-xs font-medium uppercase tracking-wider mb-2">Cover Image</label>
                <input
                  ref={fileRef}
                  type="file"
                ...
                {preview ? (
                  ...
                  >
                    <svg className={`w-8 h-8 transition ${dragging ? "text-rose-400" : "text-gray-600 group-hover:text-rose-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    <span className={`text-sm transition ${dragging ? "text-rose-300" : "text-gray-600 group-hover:text-gray-400"}`}>
                      {dragging ? "Drop image here" : "Click or drag image from desktop"}
                    </span>
                    <span className="text-xs text-gray-700">JPEG, PNG, WebP or GIF · Max 5 MB</span>
                  </button>
                )}
              </div>
              ...
              <div className="flex items-center gap-4 pt-2">
                ...
                <Link
                  to="/"
                  className="px-6 py-3 border border-white/10 hover:border-white/20 text-gray-400 hover:text-white rounded-xl transition text-center"
                >
                  Cancel
                </Link>
              </div>
