...
import type { CreateReplyInput, CreateReviewInput, UpdateReviewInput, VoteReviewInput } from "@/lib/types";
...
export const createReview = async (userId: string, input: CreateReviewInput) => {
  ...
  await logActivity(userId, "REVIEW_CREATED", titleId, { reviewId: review.id, rating });
  return review;
};
...
export const updateReview = async (
  ...
  await logActivity(userId, "REVIEW_UPDATED", review.titleId, { reviewId: updated.id });
  return updated;
};
...
export const deleteReview = async (
  ...
  await recalculateAverage(review.titleId);
  await logActivity(userId, "REVIEW_DELETED", review.titleId, { reviewId });
};
...
export const createReply = async (userId: string, reviewId: string, input: CreateReplyInput) => {
  ...
  await logActivity(userId, "REVIEW_REPLIED", parentReview.titleId, {
...
const refreshHelpfulCounts = async (reviewId: string) => {
  ...
  const helpfulCount = grouped.find((g: { value: number; _count: { _all: number } }) => g.value === 1)?._count._all ?? 0;
  const notHelpfulCount = grouped.find((g: { value: number; _count: { _all: number } }) => g.value === -1)?._count._all ?? 0;
